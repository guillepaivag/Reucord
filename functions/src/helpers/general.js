...
function dataConversionPd(pddatosDatos, pdDatos){
    ...
    pddatos.PDFECHA = pddatos.PDFECHA ? new Date(`${trim(pddatos.PDFECHA)}T13:00:00.000Z`) : null
    ...
    pddatos.FECHA_REC = pddatos.FECHA_REC ? new Date(`${trim(pddatos.FECHA_REC)}T13:00:00.000Z`) : null
    ...
    for (let i = 0; i < pd.length; i++) {
        ...
        pd[i].PDFECHA = element.PDFECHA ? new Date(`${trim(element.PDFECHA)}T13:00:00.000Z`) : null
        pd[i].FECHATRA = element.FECHATRA ? new Date(`${trim(element.FECHATRA)}T13:00:00.000Z`) : null
...
function dataConversionPs(psdatos, psdatosTrabajos){
    ...
    ps.FECHA_REC = ps.FECHA_REC ? new Date(`${trim(ps.FECHA_REC)}T13:00:00.000Z`) : null
    ...
    for (let i = 0; i < psTrabajos.length; i++) {
        ...
        psTrabajos[i].FECHATRABA = element.FECHATRABA ? new Date(`${trim(element.FECHATRABA)}T13:00:00.000Z`) : null
        ...
        console.log('psTrabajos[i].FECHATRABA.toISOString()', psTrabajos[i].FECHATRABA.toISOString())
        console.log('psTrabajos[i].FECHATRABA.getTime()', psTrabajos[i].FECHATRABA.getTime())
    }
...
function dataConversionReclamos(reclamoDatos){
    ...
    reclamo.FECHATRASM = reclamo.FECHATRASM ? new Date(`${trim(reclamo.FECHATRASM)}T13:00:00.000Z`) : null
...
function dataConversionSuspensionPD (suspensionDatos) {
    ...
    suspension.FECHATRAS = suspension.FECHATRAS ? new Date(`${trim(suspension.FECHATRAS)}T13:00:00.000Z`) : null
    suspension.HORATRAS = suspension.HORATRAS ? trim(suspension.HORATRAS) : null
...
function dataConversionSuspensionPS (suspensionDatos) {
    let suspension = JSON.parse(JSON.stringify(suspensionDatos))
    ...
    for (let i = 0; i < suspension.ITEMS_INFO.length; i++) {
        ...
        suspension.ITEMS_INFO[i].PDITEM = element.PDITEM ? parseInt(trim(element.PDITEM)) : null
        suspension.ITEMS_INFO[i].DESCRIP = element.DESCRIP ? trim(element.DESCRIP) : null
    }

    return suspension
}

function formatoRespuesta ( status, respuestaEnviar ) {

    const { codigo, mensaje, resultado } = respuestaEnviar
...
module.exports = {
    ...
    dataConversionSuspensionPD,
    dataConversionSuspensionPS,
    formatoRespuesta
}